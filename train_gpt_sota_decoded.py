...
def _compile(obj,role='train',**kwargs):
	env_key={'train':'TORCH_COMPILE_MODE','eval':'TORCH_COMPILE_MODE_EVAL','ttt':'TORCH_COMPILE_MODE_TTT'}.get(role,'TORCH_COMPILE_MODE')
	mode=os.environ.get(env_key,'').strip()
	if mode.lower() in {'0','off','false','disable','disabled','none'}:
		try:log(f"compile:{role} disabled")
		except Exception:pass
		return obj
	if mode and mode.lower()!='default':
		kwargs['mode']=mode
		try:log(f"compile:{role} mode={mode}")
		except Exception:pass
	else:
		try:log(f"compile:{role} mode=default")
		except Exception:pass
...
def train_model(h,device,val_data):
	...
	while True:
		...
		with torch.no_grad():
			_eqa_frac=float(os.environ.get('EMA_QUANT_AWARE_FRAC','0.0'));_eqa_active=_eqa_frac>0 and frac>=(1.-_eqa_frac)
